...
export function ATSScoreBadge({ score, matchedKeywords, missingKeywords, label }: ATSScoreBadgeProps) {
      ...
      {expanded && (
        <div className="absolute top-full mt-2 right-0 z-50 w-80 max-h-72 overflow-y-auto rounded-xl border border-gray-700 bg-gray-900 shadow-xl p-4">
          ...
          {missingKeywords.length > 0 && (
            <div>
              <p className="text-[10px] font-medium text-red-400 mb-1.5">Missing ({missingKeywords.length})</p>
              <div className="flex flex-wrap gap-1">
                {missingKeywords.map((kw) => (
                  <span key={kw} className="px-2 py-0.5 text-[11px] rounded-full bg-red-500/15 text-red-400 border border-red-500/20">
                    {kw}
                  </span>
                ))}
              </div>
...
export function ATSScoreMini({ score }: { score: number }) {
  const color = scoreColor(score);
  return (
    <span className={`text-xs font-bold ${color.text} ${color.bg} border ${color.border} px-2 py-0.5 rounded-full`}>
      {score}
    </span>
  );
}
