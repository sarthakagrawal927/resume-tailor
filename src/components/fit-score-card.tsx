...
function scoreColor(score: number) {
  if (score >= 70) return { bar: 'bg-[var(--accent)]', text: 'text-[var(--accent)]', bg: 'bg-[var(--accent)]/10', border: 'border-[var(--accent)]/20' };
  if (score >= 40) return { bar: 'bg-amber-400', text: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/20' };
  return { bar: 'bg-red-400', text: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/20' };
}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <p className="text-xs font-bold text-[var(--muted-foreground)] uppercase tracking-widest">Job Fit Score</p>
            <p className={`text-xl font-black ${color.text} leading-tight`}>{fitScore.overall_score}<span className="text-sm font-bold opacity-50">/100</span></p>
          </div>
        </div>
        <svg
        ...
        <div className="px-4 pb-4 space-y-4 border-t border-[var(--border)]/30">
          ...
          <div className="grid grid-cols-2 gap-3">
            {fitScore.strengths.length > 0 && (
              <div>
                <p className="text-[10px] font-black text-[var(--accent)] uppercase tracking-widest mb-1.5">Strengths</p>
                <ul className="space-y-1">
                  {fitScore.strengths.map((s, i) => (
                    <li key={i} className="text-[11px] text-[var(--muted-foreground)] leading-snug flex gap-1.5">
            ...
            {fitScore.gaps.length > 0 && (
              <div>
                <p className="text-[10px] font-black text-amber-400 uppercase tracking-widest mb-1.5">Gaps</p>
                <ul className="space-y-1">
          ...
          {fitScore.recommendation && (
            <div className="pt-2 border-t border-[var(--border)]/20">
              <p className="text-[10px] font-black text-[var(--primary)] uppercase tracking-widest mb-1">Recommendation</p>
              <p className="text-xs text-[var(--muted-foreground)] leading-relaxed">{fitScore.recommendation}</p>
            </div>
          )}
        </div>
      )}
...
export function FitScoreBadge({ score }: { score: number }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-xs font-black ${color.text} ${color.bg} border ${color.border} px-2.5 py-0.5 rounded-full`}>
